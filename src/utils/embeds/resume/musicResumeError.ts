 ...
 * @returns 
 */
export default function musicResumeErrorEmbed(embedData: InteractionEmbed, song: Queue): EmbedBuilder {
    const { interaction, client } = embedData;

    return new EmbedBuilder()
        .setAuthor({ name: interaction.user.displayName, iconURL: interaction.user.displayAvatarURL() ?? '' })
        .setColor(PinkColor)
        .setDescription(`เกิดข้อผิดพลาดในการหยุดชั่วคราวของเพลง:・${song.current?.info.title}`)
