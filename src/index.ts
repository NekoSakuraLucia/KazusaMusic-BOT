...
import type { KazusaClient, Command } from 'src/types';
class KazusaMusicBot {
    ...
    private async onReady(): Promise<void> {
        console.log(
            `${chalk.green(
                `[${chalk.white('KazusaMusic')}] -> [${chalk.white(
                    'INFO'
                )}] -> ${chalk.magenta(
        ...
        this.client.user?.setActivity('ไม่รู้จะเขียนอะไร', {
            ...
            try {
                    ...
                    console.log(
                        chalk.green(
                            `[${chalk.white('KazusaMusic')}] -> [${chalk.white(
                                ...
                                `Loading Slash Commands ${command.data.name} !`
                            )}`
                        )
                    );
                } else {
                    console.log(
                        chalk.green(
                            `[${chalk.white('KazusaMusic')}] -> [${chalk.white(
                                ...
                                `Loading Slash Failed ${file}: Missing data property`
                            )}`
                        )
                    );
                }
                ...
                console.log(
                    chalk.green(
                        `[${chalk.white('KazusaMusic')}] -> [${chalk.white(
                            'INFO'
                        )}] -> ${chalk.red(`Loading Slash Failed ${file}`)}`
                    )
                );
            }
        }
        ...
        try {
            ...
            console.log(
                chalk.green(
                    `[${chalk.white('KazusaMusic')}] -> [${chalk.white(
                        'INFO'
                    )}] -> Loading All Commands Successfully !`
                )
            );
        } catch (error) {
            console.error(error);
